...
function MgUpload() {
	...
	function loadImgPlate(options) {
		...
		var addLiFile = $(imgPlate).find('.mg-pic-file');
		$(addLiFile).on('touchstart', function() {
			chooseImg({
		...
		var haveFile = $(imgPlate).find('.mg-pic-file');
		if (haveFile) {
			$(haveFile).remove();
		}
		var addLi = $(imgPlate).find('.mg-pic-add');
		$(imgPlate).find('.mg-pic-add').show();
		var addLiFile = document.createElement("input");
		addLiFile.setAttribute('type', 'file');
		addLiFile.setAttribute('multiple', 'multiple');
		addLiFile.setAttribute('class', 'mg-pic-file');
		addLiFile.setAttribute('id', _options.colunmId);
		$(addLi).append($(addLiFile));
		$(addLiFile).on('touchstart', function() {
			chooseImg({
				colunmId: _options.colunmId,
				limit: _options.limit
		...
		//var selectedImgs = $(imgPlate).find('input:hidden[name="' + _options.colunmId + '"]'); //jq 
		...
		if (selectedSize > 0) {
			$.each(selectedImgs, function(i, _input) {
				...
				if ($(_input).attr('name') == _options.colunmId && typeof(_options.colunmId) != 'undefined') {
					...
					_deltip.setAttribute('class', 'mg-pic-del');
					$(_li).append(_deltip);
	...
	function chooseImg(options) {
		...
		var filechooser = $('#' + chooser.colunmId);
		$(filechooser).on('change', function() {
			try {

				var files = Array.prototype.slice.call(this.files);
				var fsize = $(imgPlate).find('li').not('.mg-pic-add').length;
				if (fsize + this.files.length > chooser.limit) {
					msg({
						'type': 'alert',
