...
export class AjaxTip extends Component {
	...
	render(){
		return (
			<div className="ajax-loading"><img src="../i/loading.gif" /></div>
		)
	}
}
...
export class MgHeader extends Component {
	...
	handleCircleClick(){
		let newCircleBtn = this.state.circleBtn ? false : true;
		this.setState({circleBtn: newCircleBtn});
	}
	render(){
		return (
			<header className="header">
				{
					this.props.data.gotoUrl ? <div className="header-l"><a className="header-back" href={this.props.data.gotoUrl}><i className="icon icon-back">&#xe600;</i></a></div> : ''
				}
				{
					this.props.data.gotoBack ? <div className="header-l"><a className="header-back" href="javascript:history.back(-1);"><i className="icon icon-back">&#xe600;</i></a></div> : ''
				}
				{
					this.props.data.headerTitle ? <div className="header-c"><strong>{this.props.data.headerTitle}</strong></div> : 
					(
						<div className="header-c">
							<div className="header-c-search">
				...
				{
					this.props.data.headerRight ? <div className="header-r"></div> : ''
				}
				{
					this.props.data.orderCircle ? (
						<div className="header-r">
							<div className="tps">
			                    <div className="threecircle" onClick={this.handleCircleClick.bind(this)}>
			                        <span></span>
			                    </div>
			                    <div className={this.state.circleBtn ? "tps-dialog show" : "tps-dialog"}>
			                        <em className="tps-dialog-triangle"></em>
			                        <ul>
										{
											this.props.cancelBtn ? <li><a href="javascript:;" className="tps-cancel-order" onClick={this.props.handleCancelOrder.bind(this)}><i className="icon icon-tps-money">&#xe60d;</i>退款</a></li> : ''
										}
                						<li><a href="javascript:;" className="tps-mango-tel" onClick={this.props.handleTel.bind(this)} ><i className="icon icon-tps-custome">&#xe607;</i>芒果客服</a></li>
			                        </ul>
			                    </div>
			                </div>
						</div>
	...
	render(){
		return (
			<div className="dialog" id="dialog">
				<div className="dialog-mask" id="dialog-mask" ></div>
				<div className="dialog-msg" id="dialog-msg">
					<div className="dialog-title">
						<strong className="dialog-title-content"></strong>
					</div>
					<div className="dialog-msg-content">{this.state.text}</div>
					<div className="dialog-bottom">
						<span className="dialog-bottom-cancel" onClick={this.handleFailed.bind(this)}>{this.state.btnNo}</span>
						<span className="dialog-bottom-sure" onClick={this.handleSuccess.bind(this)}>{this.state.btnOK}</span>
					</div>
				</div>
			</div>
		)
