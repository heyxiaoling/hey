			...
			success: function(e) {
				if(e.code == 1) {
					for(var i = 0; i < e.data.images.length; i++) {
						...
						var ele = $("<img class='title-pic' src=" + e.data.images[i].imageUrl + " Data-src=" + e.data.images[i].value + ">");
						$(".swiper-slide").eq(i).append(ele);
					};
					for(var i = 0; i < e.data.hot.length; i++) {
						var ele = $("<dl class='visa-nav-content' Data_Id='" + e.data.hot[i].countryId + "'><dt class='visa_nav_content_pic'><img src=" + e.data.hot[i].image + "></dt><dd" +
							" " + " class='visa-nav-content-title'>" + e.data.hot[i].name + "</dd></dl>");
